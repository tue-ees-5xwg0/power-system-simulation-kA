...
def create_graph(power_grid: Dict[str, List[Dict]]) -> nx.Graph:
    ...
    if not has_source_id(nodes, source[0]["node"]):
    ...
    G.graph["source_node_id"] = source[0]["node"]
...
def filter_disabled_edges(graph, remove_sym_loads=False):
    ...
    if remove_sym_loads:
        ...
        enabled_edges = [
            (u, v, d)
            for u, v, d in graph.edges(data=True)
            if (d.get("from_status") != 0 and d.get("to_status") != 0)
        ]
...
def set_edge_enabled_status(graph: nx.Graph, edge_id: int, status: bool):
    for u, v, d in graph.edges(data=True):
        if d.get("id") == edge_id:
            if (d.get("from_status") == 0 or d.get("to_status") == 0) and (status == False):
...
def find_downstream_vertices(graph: nx.Graph, edge_id: int) -> List[int]:
    edge_ids = [edge.get("id") for _, _, edge in graph.edges(data=True)]
    edge_data = None
    edge_vertices = None
